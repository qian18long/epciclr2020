...
def mlp_model_agent_p(input, num_outputs, scope, index, n_adv=2, n_good=5, n_land=6, num_units=64, with_action=False, share_weights=False, reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if n_good != 1:
            ...
            other_good_beg = self_dim+2*(n_adv)
            other_good_in = input[:, other_good_beg:other_good_beg + other_good_dim]
            other_good_ins = []
            for i in range(n_good-1):
                pos = other_good_in[:, i*2:(i+1)*2]
                vel = other_good_in[:, 2*(n_good-1)+i*2:2*(n_good-1)+(i+1)*2]
                is_live = other_good_in[:, 4*(n_good-1)+i:4*(n_good-1)+i+1]
                if with_action:
                    tmp = tf.concat([pos, vel, is_live], axis=1)
                else:
                    tmp = tf.concat([pos, vel, is_live, self_land], axis=1)
        ...
        #wolf_mlp
        other_adv_dim = 2*(n_adv)
        other_adv_beg = self_dim
        other_adv_in = input[:, other_adv_beg:other_adv_beg + other_adv_dim]

        other_adv_ins = []
        for i in range(n_adv):
            pos = other_adv_in[:, i*2:(i+1)*2]
            if not with_action:
                pos = tf.concat([pos, self_land], axis=1)
            other_adv_ins.append(pos)
        ...
        # print("mlp_model_agent_p", len(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tf.get_variable_scope().name)))
...
def mlp_model_adv_p(input, num_outputs, scope, index, n_adv=2, n_good=5, n_land=6, num_units=64, with_action=False, share_weights=False, reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # other sheep mlp
        other_good_dim = (2+2+1)*n_good
        other_good_beg = self_dim+2*(n_adv-1)
        other_good_in = input[:, other_good_beg:other_good_beg + other_good_dim]
        other_good_ins = []
        for i in range(n_good):
            pos = other_good_in[:, i*2:(i+1)*2]
            vel = other_good_in[:, 2*n_good+i*2:2*n_good+(i+1)*2]
            is_live = other_good_in[:, 4*n_good+i:4*n_good+i+1]
            if with_action:
                tmp = tf.concat([pos, vel, is_live], axis=1)
            else:
                tmp = tf.concat([pos, vel, is_live, self_land], axis=1)
...
def mlp_model_agent_q(input, num_outputs, scope, index, n_adv=3, n_good=5, n_land=6, num_units=64, share_weights=False, reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # split self obs
        length_wolf = (n_land+n_good+n_adv)*2+(n_good)*(1+2)+3
        length_sheep = length_wolf-3
        ...
        if n_good != 1:
            ...
            if basic:
                ...
                for i in range(n_good-1):
                    true_id = i if i < index - n_adv else i + 1
                    with tf.variable_scope("good{}".format(0 if share_weights else true_id), reuse=reuse):
                        out = mlp_model_agent_p(other_good_ins[i], num_test, 'mlp', true_id + n_adv, n_adv=n_adv,
        ...
        if basic:
            ...
            for i in range(n_adv):
                with tf.variable_scope("adv{}".format(0 if share_weights else i), reuse=reuse):
                    out = mlp_model_adv_p(other_adv_ins[i], num_test, 'mlp', i, n_adv=n_adv, n_good=n_good,
...
def mlp_model_adv_q(input, num_outputs, scope, index, n_adv=3, n_good=5, n_land=6, share_weights=False, num_units=64, reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if basic:
            ...
            for i in range(n_good):
                with tf.variable_scope("good{}".format(0 if share_weights else i), reuse=reuse):
                    out = mlp_model_agent_p(other_good_ins[i], num_test, 'mlp', i + n_adv, n_adv=n_adv, n_good=n_good,
        ...
        if n_adv != 1:
            ...
            if basic:
                ...
                for i in range(n_adv-1):
                    true_id = i if i < index else i + 1
                    with tf.variable_scope("adv{}".format(0 if share_weights else true_id), reuse=reuse):
                        out = mlp_model_adv_p(other_adv_ins[i], num_test, 'mlp', true_id, n_adv=n_adv, n_good=n_good,
                                              n_land=n_land, share_weights=share_weights, num_units=num_units, with_action=True, reuse=reuse)
                        other_adv_outs.append(out)
        else:
            other_adv_outs = []
