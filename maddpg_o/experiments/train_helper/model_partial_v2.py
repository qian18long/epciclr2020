...
def mlp_model_agent_p(input, num_outputs, scope, index, n_adv=2, n_good=5, n_land=6, num_units=64, with_action=False, share_weights=False, reuse=None, attention_handle=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if n_good != 1:
            ...
            for i in range(n_good-1):
                ...
                with tf.variable_scope("good{}".format('' if share_weights else true_id), reuse=reuse):
        ...
        other_adv_outs = []
        for i in range(n_adv):
            with tf.variable_scope("adv{}".format('' if share_weights else i), reuse=reuse):
        ...
        if attention_handle is not None:
            attention_handle(other_good_out_attn, other_adv_out_attn)
...
def mlp_model_adv_p(input, num_outputs, scope, index, n_adv=2, n_good=5, n_land=6, num_units=64, with_action=False, share_weights=False, reuse=None, attention_handle=None):
    # This model takes as input an observation and returns values of all actions
    if reuse is None:
        reuse = tf.AUTO_REUSE if share_weights else False
    with tf.variable_scope(scope, reuse=reuse):
