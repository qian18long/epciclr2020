...
class Scenario(BaseScenario):
    def __init__(self, n_good, n_adv, n_landmarks, n_food, n_forests):
        self.n_good = n_good
        self.n_adv = n_adv
    ...
    def reward_all_in_once(self, agent, world):
        ...
        for i, agent_new in enumerate(agents_live_adv):
            ...
            reward_live_adv[i] -= 0.1 * min([np.sqrt(np.sum(np.square(food.state.p_pos - agent_new.state.p_pos))) for food in world.food])
            # collision reward:
            good_collide_id = []
            num_collide = 0
            ...
            if num_collide>=2:
                ...
                for id in good_collide_id:
                    reward_live_good[id] += 6/num_collide
                agent_new.live = 0

            # shape to encourage collision:
            if shape:
                if len(agents_live_good)>0:
                # for good in agents_live_good:
                    reward_live_adv[i] -= 0.02*min(np.sqrt(np.sum(np.square(agent_new.state.p_pos - good.state.p_pos)))for good in agents_live_good)
            # eat food reward
        ...
        for i, agent_new in enumerate(agents_live_good):
            ...
            reward_live_good[i] -= 0.1 * min([np.sqrt(np.sum(np.square(food.state.p_pos - agent_new.state.p_pos))) for food in world.food])
            # collision reward:
            adv_collide_id = []
            num_collide = 0
            if agent_new.collide:
            ...
            if num_collide>=2:
                reward_live_good[i] -= 6
                for id in adv_collide_id:
                    reward_live_adv[id] += 6/num_collide
                agent_new.live = 0
            ...
            if shape:
                ...
                if len(agents_live_adv)>0:
                    reward_live_good[i] -= 0.02*min(np.sqrt(np.sum(np.square(agent_new.state.p_pos - adv.state.p_pos))) for adv in agents_live_adv)

            # eat food reward
            for i_food, food in enumerate(world.food):
                if self.is_collision(agent_new, food):
                    reward_live_good[i] += 1
                    food_id.append(i_food)

        reward_all_live = reward_live_adv + reward_live_good
        j = 0
