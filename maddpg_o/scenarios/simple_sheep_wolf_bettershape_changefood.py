...
class Scenario(BaseScenario):
    def __init__(self, n_good, n_adv, n_landmarks, n_food, n_forests, no_wheel, wolf_speed=.5):
        self.n_good = n_good
        self.n_adv = n_adv
    ...
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.accel = 4.0 * self.wolf_speed if agent.adversary else 4.0
    ...
    def adversary_reward(self, agent, world):
        ...
        if shape:
            ...
            if not (len(l1) == 0) and not self.no_wheel:
                rew -= 0.1 * min(l1)
        i_showmore = 0
        agent.eaten = False
        if agent.collide:
