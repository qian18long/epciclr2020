...
class Scenario(BaseScenario):
    def __init__(self, n_good, n_adv, n_landmarks, n_food, n_forests, alpha):
        self.n_good = n_good
        self.n_adv = n_adv
    ...
    def reward_all_in_once(self, agent, world):
        ...
        for i, agent_new in enumerate(agents_live_adv):
            ...
            if len(agents_live_good)>0:
                if shape_wolf:
                    reward_live_adv[i] -= 0.1* min(np.sqrt(np.sum(np.square(agent_new.state.p_pos - good.state.p_pos)))for good in agents_live_good)
        ...
        for i, agent_new in enumerate(agents_live_good):
            ...
            if shape_sheep:
                reward_live_good[i] += 0.05*min(np.sqrt(np.sum(np.square(agent_new.state.p_pos - adv.state.p_pos))) for adv in agents_live_adv)
